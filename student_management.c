...
int main() {
    ...
    while (running) {
        ...
        switch (choice) {
            ...
            case 6:
                printf("\n  [OK] Thank you for using Student Management System!\n");
...
void addStudent() {
    ...
    if (recordExists(s.rollNo)) {
        printf("\n  [ERROR] Roll Number %d already exists! Use Update to modify.\n", s.rollNo);
        return;
    }
    ...
    if (fp == NULL) {
        printf("\n  [ERROR] Error opening file! Cannot save record.\n");
        return;
    }
...
void displayAll() {
    ...
    FILE *fp = fopen(FILE_NAME, "rb");
    if (fp == NULL) {
        printf("\n  [INFO]  No records found. Please add students first.\n");
        return;
    }
...
void updateStudent() {
    ...
    for (int i = 0; i < count; i++) {
        if (records[i].rollNo == rollNo) {
            ...
            if (records[i].marks < 0 || records[i].marks > 100) {
                printf("\n  [ERROR] Invalid marks! Update cancelled.\n");
                return;
            }

            records[i].grade = calculateGrade(records[i].marks);
            printf("\n  [OK] Record updated! New Grade: %c\n", records[i].grade);
            break;
        }
    }
    ...
    if (fw == NULL) {
        printf("\n  [ERROR] Error saving updated records!\n");
        return;
    }
...
void deleteStudent() {
    ...
    FILE *fw = fopen(FILE_NAME, "wb");
    if (fw == NULL) {
        printf("\n  [ERROR] Error saving records after deletion!\n");
        return;
    }
